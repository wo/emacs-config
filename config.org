...
** Keyboard shortcut for closing other windows
...
Make background transparent and scale font-size up a little:
...
Configure new org-internal citations:
...
A function to update my bibtex file exported from zotero:
...
*** Change title -- update buffer name, filename, and links

From https://org-roam.discourse.group/t/does-renaming-title-no-longer-renames-the-filename/2018/2
...
(defun my-org2tex ()
  ...
  (save-buffer)
  (call-process-shell-command
   (format "python3 /home/wo/notes/papers/org2tex.py %s"
           (shell-quote-argument (buffer-file-name)))
   nil 0)
  (revert-buffer t t t)
)
...
Extract and process handout from org file
