        ...
        '((" " "Today"
           ((agenda "" ((org-agenda-span 1)
            ...
            (alltodo "" ((org-agenda-overriding-header "")
                         (org-super-agenda-groups
                          '(
                            (:name "Write" :and(:tag "write" :todo "ACTV"))
                            (:name "Read" :and(:tag "read" :todo "ACTV"))
                            (:name "Programming" :and(:tag "prog" :todo "ACTV"))
                            (:name "Other tasks" :todo "ACTV")
                            (:name "Routines" :tag "routine")
                            (:name "Deadlines" :deadline t :order 2)
                            (:name "Next: Write" :and(:tag "write" :todo "NEXT"))
                            (:name "Next: Read" :and(:tag "read" :todo "NEXT"))
                            (:name "Next: Programming" :and(:tag "prog" :todo "NEXT"))
                            (:name "Next: Other tasks" :todo "NEXT")
...
(defun my-org2latex ()
  ...
  (let ((output-dir (concat (file-name-directory (buffer-file-name))
    ...
    (async-shell-command
     (format "org2pdf --latex --template ~/notes/papers/.template.tex %s %s"
             (shell-quote-argument (buffer-file-name))
