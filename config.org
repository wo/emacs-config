...
** A function to remove first names from literature note titles

#+begin_src emacs-lisp :tangle yes
(defun my-remove-first-names-region (start end)
  "Remove first names from a region of text in 'Lastname, Firstname' format."
  (interactive "r") ; 'r' means this function operates on a region
  (let* ((input-string (buffer-substring start end))
         (result (replace-regexp-in-string ", [^ ]+\\b" "" input-string)))
