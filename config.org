...
** Partially fix broken M-RET and C-RET behaviour in org files
...
;;   :custom
;;   (citar-bibliography '("~/notes/literature.bib")))
#+end_src
...
** A function to update the references database from zotero
...
** A function for inserting links to headings
...
** Functions to convert org notes into PDF papers or handouts

Convert org to LaTeX and PDF and open the PDF:
...
(map!
 :leader
 :desc "open org note for literature item" "n e" #'citar-open-notes
 )
#+end_src
